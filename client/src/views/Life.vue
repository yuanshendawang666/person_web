...
async function fetch() { const { data } = await postAPI.list({ category:'life', sort:sort.value }); posts.value = data.posts }
onMounted(fetch)
</script>
<template>
  <div class="page-header">
    <div class="header-row"><h1 class="page-title">🌿 生活</h1><div class="sort-bar"><button v-for="o in sortOptions" :key="o.value" class="sort-btn" :class="{active:sort===o.value}" @click="sort=o.value;fetch()">{{o.label}}</button></div></div>
